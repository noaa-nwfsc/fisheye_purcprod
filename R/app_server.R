...
app_server <- function(input, output, session) {
    ...
    met_plot_df <- reactive({
        ...
        if (
            ...
            input$tab_top == "Metric" &&
                input$tab_bottom == "Production Activities"
        ) {
            df <- met_prac |>
                dplyr::filter(
                    # "Metric" tab filters
            ...
            if (input$tab_bottom == "Production Activities") {
                df <- prod_prac |>
                    dplyr::filter(
                        ...
                        .data$production_activity %in%
                            c(
                                other_tabs_inputs()$prodac,
                                other_tabs_inputs()$osps
                            )
                    ) |>
                    dplyr::mutate(
                        # adjusting price related cols for deflation value
            ...
            if (input$tab_specs_bottom == "Product Type") {
                df <- specs_prod |>
                    dplyr::filter(
                        ...
                        .data$production_activity %in%
                            c(specs_inputs()$specs, specs_inputs()$os),
                        # bottom tab filters
    ...
    output$exp_plot_ui <- renderPlot({
        ...
        if (input$tab_top == "Metric") {
            plot_func(
                ...
                data = met_plot_df(),
                lab = NULL,
                group = if (input$tab_bottom == "Production Activities") {
                    "production_activity"
                } else {
                    "characteristic"
                },
            ...
            plot_func(
                ...
                group = if (input$tab_specs_bottom == "Product Type") {
                    "type"
                } else {
                    "characteristic"
                },
